...
#    perturbation_size = int(np.mean(perturbation_size)) if len(perturbation_size) > 0 else 
    ...
    expl_strat_params = {'beta' : lambda t : 0.5 * np.log(t + 2) / t,  'seed' : 1373 + 123*r}
    ...
    with open(f"{out_dir}/zexe_lan_{str(file_path).split('/')[-1].split('.')[0]}_{model_name}_{num_sections}_{r}.txt", "a") as f:
