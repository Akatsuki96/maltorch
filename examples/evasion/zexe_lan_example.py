...
#    for (model_name, model) in networks.items():
    ...
    with open(f"{out_dir}/zexe_lan_{model_name}_{r}.txt", "w") as f:
        for i in range(len(file_paths)):
            zexe_attack = ZEXESectionInjection(
                query_budget = 1000,
                perturbation_size = 4096 * 100,# * 100,
                how_many_sections = 100,#00,
                num_directions = 1, # aka population size
                min_stepsize = 1.0,
                max_stepsize = 1000.0,
                popsize = 10, beta = lambda k : 10.00 * np.log(k + 1) / (k + 1),
                method="zexers",
                expl_strat = 'langevin',
