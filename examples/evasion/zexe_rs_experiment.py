...
def run_experiment(fpath_rep):
    ...
    expl_start = ExplorationStrategyID.RANDOM
    expl_strat_params = {'eps' : 0.01,  'seed' : 1373 + 123*r}
    ...
    with open(f"{out_dir}/zexe_rs_{str(file_path).split('/')[-1].split('.')[0]}_{model_name}_{num_sections}_{r}.txt", "a") as f:
